...
| **Scripts** | `scripts/vendor-frontend.js` copies Bootstrap/GSAP into `vendor/`. `scripts/cache-exif-dates.php` backfills capture dates from EXIF into `images/photo/.cache/`. `scripts/cache-weather-hours.php` prewarms Open-Meteo ERA5 hours for those capture days. `scripts/prune-exif-tmp.php` clears leftover EXIF temp JPEGs. |
...
**Requirements:** PHP 8.2+, Composer, Node.js 20+ (for frontend vendor copies).
